...
#databases=$(find /private/var/mobile/Containers/Data/Application/$AppID -name '*.db' -o -name '*.sqlite' -o -name '*.sqlite3' -type f)
...
DataDir=$(plutil -key User -key "$PackageName" -key EnvironmentVariables -key HOME /User/Library/MobileInstallation/LastLaunchServicesMap.plist)
databases=$(find "$DataDir" -name '*.db' -o -name '*.sqlite' -o -name '*.sqlite3' -type f)
if [ -n "$databases" ]
then
